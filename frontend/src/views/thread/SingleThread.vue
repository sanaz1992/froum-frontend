<template>
  <v-container>
    <v-row
        justify="center"
        content="center"
    >
      <v-col
          cols="12"
          md="8"
      >
        <v-card
            outlined
        >
          <v-card-title>
            <v-col cols="6" md="6">
              <h2>{{ thread.title }}</h2>
            </v-col>
            <v-col cols="6" md="6" class="text-right">
              <v-btn color="red text-white" v-if="userCanDelete" @click="deleteThread">delete</v-btn>
            </v-col>

          </v-card-title>
          <v-card-text>
            <v-row>
              <v-col>
                <p>{{ thread.user.name }}</p>
              </v-col>
              <v-col>
                <p class="text-right">{{ thread.created_at }}</p>
              </v-col>
            </v-row>
            <p> {{ thread.content }} </p>
          </v-card-text>
        </v-card>
      </v-col>

      <v-col cols="12" md="8">
        <v-row>
          <v-col>
            <h2>Replies: </h2>
          </v-col>
          <v-col class="text-right">
            <v-btn ripple to="#" color="green text-white" @click="showReplyForm=!showReplyForm"
                   v-if="isAuth">
              Reply
            </v-btn>
          </v-col>
        </v-row>
      </v-col>
      <v-col cols="12" md="8" v-if="showReplyForm">
        <v-row justify="center" content="center">
          <v-col cols="6" md="6">
            <h5>submit new reply</h5>
          </v-col>
          <v-col cols="6" md="6" class="text-right">
            <v-btn color="red text-white" @click="showReplyForm=!showReplyForm">cancel</v-btn>
          </v-col>
          <v-col cols="12" md="12">
            <v-form>
              <v-textarea name="Content" label="content" v-model="replyData.content"></v-textarea>
              <v-btn color="blue text-white" @click="submitNewReply">submit</v-btn>
            </v-form>
          </v-col>
        </v-row>
      </v-col>
      <v-col cols="12" md="8">
        <v-row justify="center" content="center">
          <v-col
              cols="12"
              md="12"
              v-for="(answer,index) in thread.answers" :key="index"
          >
            <v-card
                outlined
            >
              <v-card-title>
                <v-avatar
                    color="orange"
                    size="62"
                >
                  SK
                </v-avatar>
                <v-row class="ml-2">
                  <v-col cols="12" md="12">
                    <h5>{{ answer.user.name }}</h5>
                  </v-col>
                  <v-col cols="12" md="12">
                    <small class="text-grey" style="font-size: 10pt">{{ answer.created_at }}</small>
                  </v-col>
                </v-row>
              </v-card-title>
              <v-card-text>
                {{ answer.content }}
              </v-card-text>
            </v-card>
          </v-col>
        </v-row>
      </v-col>

    </v-row>

  </v-container>
</template>

<script>
import VueMarkdown from 'vue-markdown/src/VueMarkdown'
import {deleteThreadRequest, getSingleThreadRequest, submitNewReplyForThreadRequest} from "@/requests/Threads";
import {checkAuth, getUserDataRequest} from "@/requests/Auth";

export default {
  name: "SingleThread",
  data: () => ({
    isAuth: false,
    userCanDelete: false,
    showReplyForm: false,
    thread: {
      id: null,
      title: null,
      content: null,
      created_at: null,
      best_answer_id: null,
      channel: {
        id: null,
        name: null,
      },
      user: {
        id: null,
        name: null
      },
      answers: [],
    },
    replyData: {
      thread_id: null,
      content: null
    }
  }),
  components: {
    VueMarkdown
  },
  methods: {
    fetchThread() {
      getSingleThreadRequest(this.$route.params.slug).then(res => {
        this.thread = res.data
      })
    },
    submitNewReply() {
      this.replyData.thread_id = this.thread.id;
      submitNewReplyForThreadRequest(this.replyData).then(res => {
        this.fetchThread();
        this.showReplyForm = false;
      })
    },
    deleteThread() {
      deleteThreadRequest(this.thread.id).then(res => {
        this.$router.push('/');
      })
    }

  },
  mounted() {
    this.fetchThread();
    checkAuth();
    this.isAuth = localStorage.getItem('isAuth') === 'true';
    getUserDataRequest().then(res => {
      this.userCanDelete = res.data[0].id === this.thread.user.id;
    })
  }
}
</script>
